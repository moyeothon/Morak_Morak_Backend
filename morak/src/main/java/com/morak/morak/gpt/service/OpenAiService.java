...
public class OpenAiService {
    ...
    @Transactional
    public String askQuestion(UserGptRequestDto userGptRequestDto) {
        ...
        if (userQuestion.startsWith("정답! ")) {
            ...
            if (answer.equals(defaultKeyword)) {
                ...
                conversationHistory.add("GPT: " + endGameMessage); // 대화 기록에 GPT 응답 추가
                return endGameMessage;  // 게임 종료 메시지 반환
            } else {
                return "틀렸습니다! 다시 시도해 주세요.";  // 정답이 틀린 경우
            }
        }

        String prompt = String.format(
                "사용자가 정답을 맞추는 스무고개 게임을 진행 중입니다. 키워드는 '%s'입니다. 사용자 질문: \"%s\"에 대해 Yes 또는 No로 대답해 주세요.",
        ...
        conversationHistory.add("GPT: " + botResponse);
    ...
    public String getHint() {
        if (defaultKeyword == null) {
            return "게임이 아직 시작되지 않았습니다. 먼저 새 게임을 시작해 주세요.";
        }

        if (hintCount >= 5) {
            return "힌트 제공 횟수를 초과했습니다."; // 힌트 제공 제한
        }
